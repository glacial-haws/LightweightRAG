...
def plot_evaluation(df: pd.DataFrame, temperature: float, output_dir: Path, light: bool = False) -> None:
    cols = df.columns
    llm_models = [col.split('_')[0] for col in cols if (col.endswith('_score') and not col.startswith('human_'))]
...
def benchmark_llm(df: pd.DataFrame, temperature: float, output_file: Path) -> pd.DataFrame:
    ...
    for i, llm_model in enumerate(llm_models):
        ...
        # skip models already have benchmark results
        if any(llm_model.lower() in col.lower() for col in df.columns):
            logging.info(f"Skipping {llm_model} as it is already in the dataframe")
            continue

        tqdm.pandas(desc=f"{i+1:d}/{len(llm_models):d} {llm_model} t{temperature}", total=len(df))
        
        # Use progress_apply instead of apply
        df[[f'{llm_model}_score', f'{llm_model}_response', f'{llm_model}_time_sec']] = \
            df.progress_apply(
        ...
        # save after each model, process is restartable
...
def pse_main() -> None:
    ...
    if not args.files:
        PU.print_help()
        print("Please provide path to excel file with human evaluation.")
        return

    human_eval_file = Path(args.files[0])
    temperature = 0.1
    output_file = human_eval_file if human_eval_file.name.startswith("benched_") else human_eval_file.parent / f"benched_{human_eval_file.stem}.xlsx"

    he_df = pd.read_excel(human_eval_file)
    logging.info(f"Read human evaluation data from {LC.HI}{human_eval_file}{LC.RESET}, shape: {LC.HI}{he_df.shape}{LC.RESET}")

    if not {'bleu'}.issubset(he_df.columns):
        logging.info("Calculating BLEU scores...")
        he_df['bleu'] = he_df.apply(lambda row: calc_bleu(row['expected_answer'], row['answer']), axis=1)
    if not {'rouge'}.issubset(he_df.columns):
        logging.info("Calculating ROUGE scores...")
        he_df['rouge'] = he_df.apply(lambda row: calc_rouge(row['expected_answer'], row['answer']), axis=1)
    
    benchmark_df = benchmark_llm(he_df, temperature=temperature, output_file=output_file)

    plot_evaluation(benchmark_df, temperature=temperature, output_dir=output_file.parent, light=True)
